using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Behind : MonoBehaviour
{
    public GameObject go;
    Alpha a;
    Eyex _eye;
    GameObject eyefind;

    void Start()
    {
       go = GameObject.Find("Logo_light");
       a = (Alpha)go.GetComponent(typeof(Alpha));
       eyefind = GameObject.Find("eye");
        _eye = (Eyex)eyefind.GetComponent(typeof(Eyex));
    }

    void Update()
    {
        Vector3 mousePosition = Camera.main.ScreenToWorldPoint(Input.mousePosition);
        if (mousePosition.x > 31 && mousePosition.x < 339
            && mousePosition.y > 178 && mousePosition.y < 246)
            a.change_alpha_to(255f);
        else if (_eye.transform.position.x > 31 && _eye.transform.position.x < 339
                && _eye.transform.position.y > 178 && _eye.transform.position.y < 246)
            a.change_alpha_to(255f);
        else
            a.change_alpha_to(0f);
    }
}